...
function testNormalEmail() {
  Logger.log("=== STARTING NORMAL SUBMISSION EMAIL TEST ===");

  // 1. Create Template
  var template = HtmlService.createTemplateFromFile('AdminEmail');

  // 2. Define ALL properties referenced by AdminEmail.html
  ...
  template.request = {
    situation: "Test situation details",
    goal: "Test goal details",
    timeframe: "Flexible"
  };
...
template.secEval = {
  ...
  isUrgent: false,
  spamScore: 0,
  statusText: "Passed Security Check",
  flags: [],
  reasons: []
};

  Logger.log("Security Check -> Is Spam: " + template.eval.isSpam);
  Logger.log("Security Check -> Needs Review: " + template.eval.requiresReview);

  // 3. Evaluate after properties are attached
  try {
    var htmlBody = template.evaluate().getContent();
    
    MailApp.sendEmail({
      to: "tom@rd3tech.com",
      subject: "[TEST] Submission Email Test",
      htmlBody: htmlBody
    });
